 ...
 * Detection patterns for documentation optimization
 *
 * Supports two optimization modes:
 * - 'ai': AI-only (agent-docs) - Aggressive RAG optimization
 * - 'both': Both audiences - Balance readability with AI-friendliness
...
/**
 * Documentation patterns with mode field
 * Mode values: 'ai' | 'both' | 'shared'
 */
const docsPatterns = {
  // ============================================
  // SHARED PATTERNS (apply to both modes)
  // ============================================

  /**
   * Broken internal link
   * HIGH certainty - links that don't resolve
   */
   ...
   * HIGH certainty - H1 -> H3 without H2
   */
   ...
   * HIGH certainty - code blocks without language hint
   */
  ...
  /**
   * Section too long
   * MEDIUM certainty - >1000 tokens in single section
   */
  section_too_long: {
    id: 'section_too_long',
    category: 'structure',
  ...
  // ============================================
  // AI-ONLY PATTERNS (for agent-docs)
  // ============================================

  /**
   * Unnecessary prose
   * HIGH certainty for AI docs - filler text wastes tokens
   */
  unnecessary_prose: {
   ...
   * HIGH certainty for AI docs - can be compressed
   */
  verbose_explanations: {
    ...
    check: (content) => {
      ...
      const verbosePatterns = [
        ...
        { pattern: /the majority of/gi, replacement: 'most' },
  ...
  /**
   * Suboptimal chunking
   * MEDIUM certainty - content not structured for RAG retrieval
   */
  suboptimal_chunking: {
   ...
   * MEDIUM certainty - mixed topics in single section
   */
   ...
   * MEDIUM certainty - sections lack self-contained context
   */
  ...
  /**
   * Token inefficiency suggestions
   * LOW certainty - optimization opportunities
   */
  ...
  // ============================================
  // BOTH-MODE PATTERNS (balance readability + AI)
  // ============================================

  /**
   * Missing section headers
   * MEDIUM certainty - long content without structure
   */
  missing_section_headers: {
    ...
    check: (content) => {
      ...
      for (const part of parts) {
        // Skip if this is a heading line itself
        const cleanPart = part.split('\n').slice(1).join('\n');
   ...
   * MEDIUM certainty - important info buried
   */
  poor_context_ordering: {
    ...
    check: (content) => {
      ...
      const criticalKeywords = [
        /\b(?:important|critical|must|required|warning|caution|danger)\b/gi,
        /\b(?:error|fail|break|crash|security|vulnerability)\b/gi
      ];
  ...
  /**
   * Readability with RAG suggestions
   * LOW certainty - balance suggestions
   */
   ...
   * LOW certainty - general structure advice
   */
...
/**
 * Get all patterns
 * @returns {Object} All documentation patterns
 */
 ...
 * @param {string} mode - 'ai', 'both', or 'shared'
 ...
 * @param {string} category - link, structure, code, efficiency, rag, balance
 * @returns {Object} Filtered patterns
 */
...
/**
 * Get patterns for a specific mode (including shared)
 * @param {string} mode - 'ai' or 'both'
 * @returns {Object} Patterns applicable to the mode
 */
