...
const COMMENT_SYNTAX = {
  js: { line: /^\s*\/\//, block: { start: /\/\*/, end: /\*\// } },
  python: { line: /^\s*#/, block: { start: /"""/, end: /"""/ } },
  rust: { line: /^\s*\/\//, block: { start: /\/\*/, end: /\*\// } },
  go: { line: /^\s*\/\//, block: { start: /\/\*/, end: /\*\// } }
};
  ...
  while ((match = funcPattern.exec(content)) !== null) {
    ...
    for (const line of lines) {
      ...
      if (commentSyntax.block.start.test(trimmed)) {
        commentLines++;
        if (!commentSyntax.block.end.test(trimmed.substring(trimmed.search(commentSyntax.block.start) + 2))) {
          inBlockComment = true;
        }
        continue;
      }
